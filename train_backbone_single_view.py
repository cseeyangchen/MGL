...
# 加载单视角模型
from net.backbone.st_gcn import Model as STGCN
...
def processor(modelName, viewName, optimizer_name, lr, device_list, debug,
    ...
    # 三个视角的训练集路径
    if viewName=='view1':
        train_view_data = "data/spatial_temporal_occ/ntu60_rotation45/xsub/train/train_view1_data.npy"
        train_view_label = "data/spatial_temporal_occ/ntu60_rotation45/xsub/train/train_view1_label.pkl"
        train_view = Feeder(data_path=train_view_data, label_path=train_view_label, debug=debug)
        test_view_data = "data/spatial_temporal_occ/ntu60_rotation45/xsub/val/val_view1_data.npy"
        test_view_label = "data/spatial_temporal_occ/ntu60_rotation45/xsub/val/val_view1_label.pkl"
        test_view = Feeder(data_path=test_view_data, label_path=test_view_label, debug=debug)
    elif viewName == 'view2':
        train_view_data = "data/spatial_temporal_occ/ntu60_rotation45/xsub/train/train_view2_data.npy"
        train_view_label = "data/spatial_temporal_occ/ntu60_rotation45/xsub/train/train_view2_label.pkl"
        train_view = Feeder(data_path=train_view_data, label_path=train_view_label, debug=debug)
        test_view_data = "data/spatial_temporal_occ/ntu60_rotation45/xsub/val/val_view2_data.npy"
        test_view_label = "data/spatial_temporal_occ/ntu60_rotation45/xsub/val/val_view2_label.pkl"
        test_view = Feeder(data_path=test_view_data, label_path=test_view_label, debug=debug)
    elif viewName == 'view3':
        train_view_data = "data/spatial_temporal_occ/ntu60_rotation45/xsub/train/train_view3_data.npy"
        train_view_label = "data/spatial_temporal_occ/ntu60_rotation45/xsub/train/train_view3_label.pkl"
        train_view = Feeder(data_path=train_view_data, label_path=train_view_label, debug=debug)
        test_view_data = "data/spatial_temporal_occ/ntu60_rotation45/xsub/val/val_view3_data.npy"
        test_view_label = "data/spatial_temporal_occ/ntu60_rotation45/xsub/val/val_view3_label.pkl"
        test_view = Feeder(data_path=test_view_data, label_path=test_view_label, debug=debug)
    else:
        print("We only have three views.")
        raise ValueError()
    ...
    # 每个进程一个sampler
    # train_sampler = torch.utils.data.distributed.DistributedSampler(train_view2)
    ...
    # 每个进程一个sampler
    # test_sampler = torch.utils.data.distributed.DistributedSampler(test_view2)
        ...
        for phase in ["train", "test"]:
            ...
            if phase == "train":
                ...
                for batch_index, batch_data in enumerate(data_loader["train"]):
                    ...
                    view_data = batch_data[0].float().to(device)
                    view_label = batch_data[1].float().to(device)
                    ...
                    label_frag.append(view_label.data.cpu().numpy())
                ...
                print("[train] Epoch: {}/{} lr:{} Loss:{} Top1_acc:{} Top5_acc:{}".format(epoch + 1, num_epoch,
            ...
            if (epoch + 1) % save_epoch == 0 and phase == "train":
                model_save_dir = os.path.join(save_dir_root, "model", modelName+"_"+viewName,
                                              "Epoch{}_".format(epoch + 1) + modelName + ".pt")
            ...
            if phase == "test":
                ...
                stop_time = timeit.default_timer()
                print("[test] Epoch: {}/{} Loss:{} Top1_acc:{} Top5_acc:{}".format(epoch + 1, num_epoch, test_loss,
                                                                                   acc_top1, acc_top5))
                print("Execution time: " + str((stop_time - start_time)) + "\n")
    writer.close()
...
if __name__ == "__main__":
    ...
    save_epoch = 1
    processor(modelName=modelName, viewName=viewName, optimizer_name=optimizer_name, lr=lr,
