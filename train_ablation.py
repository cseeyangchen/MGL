...
# 加载多视角融合模型
from net.ablation_study.MGL import MGL as MGL
from net.ablation_study.w_GCN import MCL as MGL_GCN
from net.ablation_study.w_LSTM import MCL as MGL_LSTM
from net.ablation_study.w_MASK import MCL as MGL_MASK
from net.ablation_study.w_MAXPOOLING import MCL as MGL_MAXPOOLING
...
def processor(modelName, optimizer_name, lr, device_list, debug,
    ...
    # 加载模型
    if modelName == 'MGL':
        model = MGL_GCN(in_channels=3,num_class=60,device_id=device_list[0],edge_importance_weighting=True,
                    graph_args={'layout': 'ntu-rgb+d', 'strategy': 'spatial'})
        model.apply(weights_init)
    elif modelName == 'MGL_GCN':
        model = MGL_GCN(in_channels=3,num_class=60,device_id=device_list[0],edge_importance_weighting=True,
                    graph_args={'layout': 'ntu-rgb+d', 'strategy': 'spatial'})
        model.apply(weights_init)
    elif modelName == 'MGL_LSTM':
        model = MGL_LSTM(in_channels=3, num_class=60, device_id=device_list[0], edge_importance_weighting=True,
                        graph_args={'layout': 'ntu-rgb+d', 'strategy': 'spatial'})
        model.apply(weights_init)
    elif modelName == 'MGL_MASK':
        model = MGL_MASK(in_channels=3, num_class=60, device_id=device_list[0], edge_importance_weighting=True,
                        graph_args={'layout': 'ntu-rgb+d', 'strategy': 'spatial'})
        model.apply(weights_init)
    elif modelName == 'MGL_MAXPOOLING':
        model = MGL_MAXPOOLING(in_channels=3, num_class=60, device_id=device_list[0], edge_importance_weighting=True,
                        graph_args={'layout': 'ntu-rgb+d', 'strategy': 'spatial'})
        model.apply(weights_init)
    else:
...
# MCL_CTRGCN lr=0.1 weight_decay=0.004 train_batch_size=32 step:[25,35] warm_step:10
# MCL_EFFICIENTGCN lr=0.1 weight_decay=0.0001 train_batch_size=64 step:[20,40]
