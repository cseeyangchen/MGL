...
from net.utils.ctrgcn.graph import Graph
import torch.nn.functional as F
import math
from net.backbone.ctrgcn import TCN_GCN_unit
...
class MCL_CTRGCN(nn.Module):
    def __init__(self, in_channels, num_class, device_id, graph_args_ctrgcn,
                 graph_args_stgcn, edge_importance_weighting):
        super().__init__()
        self.view_num = 3

        # load ctrgcn
        self.graph_ctrgcn = Graph(**graph_args_ctrgcn)
        A_ctrgcn = self.graph_ctrgcn.A
        self.data_bn = nn.BatchNorm1d(2 * in_channels * 25)

        self.ctrgcn = nn.ModuleList((
            TCN_GCN_unit(3, 64, A_ctrgcn, residual=False),
            TCN_GCN_unit(64, 64, A_ctrgcn),
            TCN_GCN_unit(64, 64, A_ctrgcn),
            TCN_GCN_unit(64, 64, A_ctrgcn),
            TCN_GCN_unit(64, 128, A_ctrgcn, stride=2),
            TCN_GCN_unit(128, 128, A_ctrgcn),
            TCN_GCN_unit(128, 128, A_ctrgcn),
            TCN_GCN_unit(128, 256, A_ctrgcn, stride=2),
            TCN_GCN_unit(256, 256, A_ctrgcn),
            TCN_GCN_unit(256, 256, A_ctrgcn)
        ))
    ...
    def forward(self, x1, x2, x3):
        ...
        for view_index in range(self.view_num):
            ...
            for ctrgcn_layer in self.ctrgcn:
                x[view_index] = ctrgcn_layer(x[view_index])
        # prediction
...
if __name__ == "__main__":
    model = MCL_CTRGCN(in_channels=3,num_class=60,device_id=0,edge_importance_weighting=True,
