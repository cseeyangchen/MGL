...
import math
from net.backbone.efficientgcn.nets import EfficientGCN
...
class MCL_EFFICIENTGCN(nn.Module):
    def __init__(self, num_class, device_id, efficientgcn_args,
                 graph_args_stgcn, edge_importance_weighting):
        super().__init__()
        self.view_num = 3

        self.efficientgcn = EfficientGCN(**efficientgcn_args)

        self.predict_spe_stage = nn.ModuleList([nn.Linear(128, num_class) for _ in range(self.view_num)])
        ...
        self.gcn_lstm_fusion_networks = GCN_LSTM_Fusion_Unit(128, 128, kernel_size_stage2, device_id, view=self.view_num)

        # 第三阶段 -- 分类器
        self.late_fusion_stage = nn.Conv2d(128, num_class, kernel_size=1)
    ...
    def forward(self, x1, x2, x3):
        ...
        for view_index in range(self.view_num):
            ...
            x[view_index] = self.efficientgcn(x[view_index])
        # prediction
...
if __name__ == "__main__":
    ...
    model = MCL_EFFICIENTGCN(num_class=60,device_id=0,edge_importance_weighting=True,
                     efficientgcn_args={"data_shape":[3, 300, 25, 2],"block_args":block_args,"stem_channel":64,"fusion_stage":2,
