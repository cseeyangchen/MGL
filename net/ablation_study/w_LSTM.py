...
"""
    MGL融合模块消融性实验（Ablation Study）
    Mask+MaxPooling+GCN+GAP
    探究时间LSTM的影响 -- w / LSTM
"""

class Temporal_Mask(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size, device_id, bias=True):
        ...
        self.conv1d = nn.Conv1d(in_channels=self.input_dim,
                                out_channels=self.input_dim * self.kernel_size,
                                kernel_size=self.kernel_size,
                                padding=self.padding,
                                bias=self.bias)

    def forward(self,input_tensor1,input_tensor2,input_tensor3, A):
        # 注意力掩码操作
        # 逐帧处理
        ...
        # GCN
        input_tensor = self.conv1d(input_tensor)
        n, kc, v = input_tensor.size()
        input_tensor = input_tensor.view(n, self.kernel_size, kc // self.kernel_size, v)
        input_tensor = torch.einsum('nkcv, kvw->ncw',(input_tensor, A))
...
class MultiConvLSTM(nn.Module):
    ...
    def __init__(self,input_dim, hidden_dim, kernel_size, device_id, bias=True, view=3):
        ...
        self.bias = bias
        self.view_num = view
        self.view_stream = Temporal_Mask(input_dim,hidden_dim,kernel_size,device_id,bias)
    ...
    def forward(self, x, A, hidden_state=None):
        ...
        for t in range(seq_len):
            h = self.view_stream(cur_input[0][:,:,t,:],cur_input[1][:,:,t,:],cur_input[2][:,:,t,:],A)
            output.append(h)

        layer_output = torch.stack(output,dim=2)
        return layer_output
...
class GCN_LSTM_Fusion_Unit(nn.Module):
    ...
    def forward(self, x, A):
        ...
        for view_index in range(self.view):
            ...
            x[view_index] = x[view_index].permute(0, 2, 1, 3).contiguous()
        x = self.lstm(x, A, hidden_state=None)  # n c t v
        n,c,t,v = x.size()
...
class MCL(nn.Module):
    def __init__(self, in_channels, num_class, device_id, graph_args,
        ...
        if edge_importance_weighting:
            edge_importance_stage1 = nn.ParameterList(
                [nn.Parameter(torch.ones(self.A.size())) for _ in self.st_gcn_networks])
            for i in range(len(self.st_gcn_networks)):
                self.register_parameter('edge_importance_stage1{}'.format(i), edge_importance_stage1[i])
    ...
    def forward(self, x1, x2, x3):
        ...
        for view_index in range(self.view_num):
            x[view_index], _ = self.st_gcn_networks[0](x[view_index], self.A * self.edge_importance_stage10)
            x[view_index], _ = self.st_gcn_networks[1](x[view_index], self.A * self.edge_importance_stage11)
            x[view_index], _ = self.st_gcn_networks[2](x[view_index], self.A * self.edge_importance_stage12)
            x[view_index], _ = self.st_gcn_networks[3](x[view_index], self.A * self.edge_importance_stage13)
            x[view_index], _ = self.st_gcn_networks[4](x[view_index], self.A * self.edge_importance_stage14)
            x[view_index], _ = self.st_gcn_networks[5](x[view_index], self.A * self.edge_importance_stage15)
            x[view_index], _ = self.st_gcn_networks[6](x[view_index], self.A * self.edge_importance_stage16)
            x[view_index], _ = self.st_gcn_networks[7](x[view_index], self.A * self.edge_importance_stage17)
            x[view_index], _ = self.st_gcn_networks[8](x[view_index], self.A * self.edge_importance_stage18)
            x[view_index], _ = self.st_gcn_networks[9](x[view_index], self.A * self.edge_importance_stage19)
