...
"""
    多视角融合方式4：对多视角特征进行最大池化操作进行融合（Max Pooling）
"""


class MaxPoolingFeature(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        # 第二阶段融合特征
        output2 = x[0].view(1*2,-1)
        for view_index in range(1,len(x)):
            output2 = torch.max(output2, x[view_index].view(1*2,-1))
        output2 = output2.view(1*2,C_new,T_new,V_new)
...
if __name__ == "__main__":
    model = MaxPoolingFeature(in_channels=3,num_class=60,edge_importance_weighting=True,
