...
"""
    多视角融合方式2：直接拼接每个视角预测的得分（Concatenate Score）
"""


class ConcatenateScore(nn.Module):
    def __init__(self, in_channels, num_class, graph_args,
        ...
        # 第二阶段融合网络
        self.predict_stage2 = nn.Conv2d(num_class*self.view_num, num_class,kernel_size=1)
    ...
    def forward(self,x1,x2,x3):
        ...
        for view_index in range(self.view_num):
            ...
            output1[view_index] = output1[view_index].view(1, 2, -1, 1, 1).mean(dim=1)
        ...
        for view_index in range(self.view_num):
            output2[view_index] = output2[view_index].view(output2[view_index].size(0), -1, 1, 1)
        output2 = torch.cat(output2, dim=1)   # 直接在通道上拼接特征
