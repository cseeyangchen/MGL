...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor1, input_tensor2, input_tensor3, cur_state, A):
        ...
        input_tensor1 = input_tensor1.permute(0, 2, 1).contiguous().to(device='cuda:'+str(0))
        input_tensor1 = input_tensor1*(torch.tanh(self.Feature_Mask1)+1)
        input_tensor2 = input_tensor2.permute(0, 2, 1).contiguous().to(device='cuda:'+str(0))
        input_tensor2 = input_tensor2 * (torch.tanh(self.Feature_Mask2) + 1)
        input_tensor3 = input_tensor3.permute(0, 2, 1).contiguous().to(device='cuda:'+str(0))
...
class GCN_LSTM_Fusion(nn.Module):
    ...
    def forward(self,x1,x2,x3):
        ...
        output3 = output3.view(1,2,-1,1,1).mean(dim=1)
...
class MCL(nn.Module):
    def __init__(self, in_channels, num_class, device_id, graph_args,
        ...
        self.graph = Graph(**graph_args)
        A = torch.tensor(self.graph.A, dtype=torch.float32, requires_grad=False)
        self.A = A.to(device='cuda:'+str(device_id))
        # As = torch.stack([A, A, A], dim=0)
        # self.register_buffer('A', A)
        # self.register_buffer('As', As)
