...
from net.utils.convlstm.convlstm import ConvLSTM
from net.backbone.st_gcn import st_gcn
import torch.nn.functional as F

"""
    多视角融合方式6：利用LSTM对多视角特征进行进行融合（LSTM Fusion）
"""
...
class LstmFeature(nn.Module):
    def __init__(self, in_channels, num_class, graph_args,
        ...
        # 第二阶段融合网络
        self.ln = nn.LayerNorm([self.view_num, 256, 75, 25], elementwise_affine=False)
        self.convlstm2d = ConvLSTM(input_dim=256, hidden_dim=256, kernel_size=(3, 3),
    ...
    def forward(self, x1, x2, x3):
        ...
        for view_index in range(len(x)):
            x[view_index] = x[view_index].unsqueeze(dim=0)
        output2 = torch.cat(x, dim=0)
        output2 = output2.permute(1,0,2,3,4).contiguous()
        output2 = self.ln(output2)
        output2, _ =self.convlstm2d(output2)
        output2 = output2[0].permute(1,0,2,3,4).contiguous()[self.view_num-1]
        output2 = F.avg_pool2d(output2,output2.size()[2:])
        output2 = output2.view(1,2,-1,1,1).mean(dim=1)
...
if __name__ == "__main__":
    model = LstmFeature(in_channels=3,num_class=60,edge_importance_weighting=True,
